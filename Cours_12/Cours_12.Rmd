...
```{r}
tweets_UniNE <- search_tweets("Unine", n = 1000)
```
...
```{r}
head(tweets_UniNE$text)
plot(tweets_UniNE$created_at)
```
...
library(ggplot2)
nouv_graph<-ggplot(tweets_UniNE, aes(x = created_at)) + geom_histogram()
nouv_graph
```
...
meilleur_graph<-ggplot(tweets_UniNE, aes(x = created_at)) + geom_histogram(binwidth = 5000) +
