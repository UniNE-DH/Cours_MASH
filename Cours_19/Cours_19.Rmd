...
```{r}
setwd("~/GitHub/Cours_DH_UniNE/Cours_19")
...
Le zoom permet de se focaliser sur un point précise: ici l'université.
...
Je peux aussi modifier le fond de carte
...
Il est possible de changer le fond de carte (_basemap_). J'ai le choix entre un grand nombre d'option
...
Je peux sauvegarder le résultat au format `json` si je veux regarder en détai le code produit:
